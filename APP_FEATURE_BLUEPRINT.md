...
| DASH-011 | 🟨 | Ride Center | 已整合行程与骑行能力入口，后续完整 Roadbook 与回放仍未完成 |
...
| ID | 状态 | 候选功能 | 最小可交付范围 |
| --- | --- | --- | --- |
| IDEA-001 | ⬜ | ADV Roadbook | 路点列表、逐点导航、偏航返回和 GPX 共享 |
| IDEA-002 | ⬜ | 完整行程回放 | 地图轨迹与速度、倾角、G 值、事件时间轴同步播放 |
...
| IDEA-007 | ⬜ | 防盗事件时间轴 | 汇总位移、震动、位置、断连和通知确认，支持证据导出 |
| IDEA-008 | ⬜ | 车队危险点与停车分享 | 由成员主动发布路障、湿滑、施工、加油和集合点，并自动过期 |
| IDEA-009 | ⬜ | XP400 指令证据库 | 按 ECU、固件版本和车型记录只读 DID、响应样本与可信等级 |
| IDEA-010 | ⬜ | 安全固件升级状态机 | 仅在真实协议确认后实现备份、预检、写入、校验、恢复和审计闭环 |
