...
当前审计结论：GATT、Credits、认证主流程、导航封装、配置封装以及 Data1～Data3 的核心换算没有重大方向性差异；已补齐已知车辆状态帧的长度校验和严格 Mock 样本，认证超时、ANCS 通道和部分写入指令仍存在实现差异。TCS、灯光、ABS 前轮速度、部分保留位和断开帧 ID 已有真车抓包证据，协议字段与代码实现仍需分开记录。
...
- [ ] **BLE-OPT-010 ANCS 完整链路**：只有在确认服务 UUID、特征、权限、通知读取边界和车辆响应后，才从原型升级为可用功能；在此之前保持开发者实验状态。
