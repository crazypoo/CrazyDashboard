...
当前审计结论：GATT、Credits、认证主流程、导航封装、配置封装以及 Data1～Data3 的核心换算没有重大方向性差异；iOS 解析器的长度校验、认证超时、ANCS 通道和部分写入指令仍存在实现差异。TCS、灯光、ABS 前轮速度、部分保留位和断开帧 ID 已有真车抓包证据，协议字段与代码实现仍需分开记录。
