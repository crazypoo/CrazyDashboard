...
| 文件 | 当前 SHA-256 |
|---|---|
| `Global/BLE/PTBluetoothManager.swift` | `27bc0720e0ba9ed42c497b340fecff02dde2c6966d0aad38d7989887c763f448` |
...
> 当前哈希已包含此前完成并经项目负责人确认的只读仪表身份出口。自 Build 45 基线起，三份文件严格零字节变化；外围功能只消费其现有公开结果。
...
- [x] 未实现协议文档没有给出完整字段的系统电话、短信和第三方通知自定义转发；后续新增的 `PTDashboardANCSProvider` 仅发送 PTSpeed 自有固定英文测试消息，不伪造系统通知已支持状态。
...
## 27. Build 45 稳定性、解耦与功能闭环计划（2026-09-04）

本轮继续使用营销版本 `2.0.8`，最终只将 PTSpeed、Widget、Watch 和 Tests 的 Build 统一推进到 `45`。原计划中的后续候选功能已全部提升为本 Build 的正式工作包；排期不等于完成，没有对应证据的条目保持 `⬜` 或 `🟨`。
...
- [ ] `PTVehicleConnectivityCoordinator` 只保留连接状态协调，对车库身份和自动保存继续提供兼容转发；具体逻辑移入单一内部车库同步组件。
- [ ] 停车保存、Widget 最终快照和会话结束只由真实“已连接 → 已断开”状态转换触发一次；冷启动初始断开、重复代理和 Mock 断开不得覆盖真实停车点。
- [ ] 使用统一定位用途集合管理骑行、导航、地图、PTT 和防盗；只有用户明确开启防盗时才使用防盗模式，最后一个用途结束后关闭持续定位。
- [ ] PTT Peer 列表、消息去重和音频时间戳使用明确隔离域；头像只在内容变化时编码落盘。
- [ ] PTT 位置共享默认关闭，开启后仅在有有效成员时发送；最快两秒一次，移动 5 米、方向变化 15°或十秒心跳才更新，位置 15 秒后过期。
- [ ] PTT Live Activity 对相同目标状态去重；没有成员时不得创建，最后成员离开、用户退出、应用恢复到无效会话时立即结束。
- [ ] `PTTripReport` 增加稳定 ID 和可选车辆 ID；骑行开始时锁定车辆归属，旧行程进入“未分配”筛选。
- [ ] 行程云端文档使用记录 ID、删除墓碑和清空时间合并；冲突副本可恢复，不能静默丢失或复活已删除记录。
- [ ] CAN JSONL 改为流式读取；摘要、候选 CAN ID、Byte/Bit diff 和事件窗口分析全部离开主线程，并提供进度、取消和损坏行报告。
- [ ] 兼容现有车辆连接、车库同步、CAN Recorder、行程历史及旧 JSON/JSONL/PTT 数据格式。

### 27.3 Build 45 新增功能定义

#### `B45-06` 出发检查（RIDE-014）

- [ ] 在现有 Ride Center 展示当前车辆、仪表/OBD 连接、燃油/续航、电瓶、保养、已有路线天气和 PTT 位置共享状态。
- [ ] 状态只分为 `ready`、`attention`、`unavailable`；阈值复用现有诊断、保养和天气模块，不创建第二套安全标准。
- [ ] 页面打开不自动连接 BLE/OBD、不启动定位、不调用天气网络；明确说明结果仅基于最近数据，不代表机械安全结论。

#### `B45-07` 轮胎/悬挂档案与保养闭环（RIDE-015、RIDE-017）

- [ ] 每辆车独立保存轮胎品牌、型号、前后轮尺寸、冷/热胎压观察值、载荷场景、里程、时间和备注，支持 bar/kPa/psi 无损换算。
- [ ] 每辆车独立保存前后预载、回弹和压缩设置；不预填未经车型手册确认的“推荐值”。
- [ ] 保养任务支持自定义名称、日期/里程周期、完成日期/里程、费用、备注和关联配件；内置名称只是模板，可删除或修改。
- [ ] XP400 模板可包含机油、机油滤芯、空气滤芯、CVT 皮带/滚珠、制动系统、冷却液、轮胎和电瓶，但周期必须由用户或已确认仪表数据给出。
- [ ] `distToMaintenance` 与每车预警里程仍是仪表保养状态来源；手工任务不能回写仪表。

#### `B45-08` 加油记录与校准续航（RIDE-016）

- [ ] 加油记录包含车辆 ID、时间、里程、加油量、金额、是否加满和备注；货币只保存用户输入，不假定币种汇率。
- [ ] 只有同一车辆连续两次“加满”且里程递增、加油量有效时，才生成满箱法油耗样本。
- [ ] Mock/异常/回拨里程不进入校准；记录保留失败原因，绝不修改仪表或车库的原始里程。
- [ ] 续航优先级为有效仪表续航、该车校准油耗、现有车型默认模型；界面显示来源和更新时间。

#### `B45-09` 综合路线骑行风险（NAV-014）

- [ ] 复用现有 Roadbook 路线、弯道几何、WeatherKit→QWeather 整路线回退结果和预计出发/到达时段。
- [ ] 报告覆盖降雨、横风、低温、高温、低能见度、夜间和连续弯道路段，并保留数据来源、采样时间和缺失项。
- [ ] 风险只能作为路线准备提示；不计算极限速度、不给出压弯建议，也不因页面展示额外启动持续定位。
- [ ] QWeather 当前初始化和回退链路保持不变，本工作包只消费已经形成的天气报告。

#### `B45-10` Watch 出发检查（SYS-014）

- [ ] 扩展现有版本化 Watch application context，加入出发检查状态、主要提醒、车辆名称和更新时间；旧 Watch 仍可忽略新增字段。
- [ ] Watch 只显示最后一次 iPhone 计算结果，没有数据时显示不可用，不自行连接仪表/OBD、不读取 iCloud。
- [ ] 仅在状态从正常变为提醒时提供一次可配置触觉，重复同步不得持续震动。

#### `B45-11` BLE 实车证据导入与报告（DEV-013）

- [ ] 在现有 Dev 模块导入项目定义的版本化 JSON 或 CSV 验证会话，校验时间、车型/固件、方向、UUID、原始帧和人工结论。
- [ ] 对照 `PTXP400BLEProtocolContract` 标记已知、长度异常、未知和需要人工确认的帧；原始数据保持可追溯。
- [ ] 导出前默认脱敏 Central UUID、仪表序列号、设备名和用户备注中的身份信息。
- [ ] 导入功能严格只读，不提供发送、重放或“一键套用”，也不根据单次样本自动把协议状态晋级为已确认。

#### `B45-12` 多车库 iCloud 同步（SYS-015）

- [ ] 云端同步车辆基础档案、保养、配件、轮胎/悬挂档案和加油记录；每条子记录具有稳定 ID、修改时间和删除墓碑。
- [ ] CoreBluetooth Central UUID、仪表序列号映射、最近连接对象和本机 Mock 绑定保留在本机绑定表，不进入 iCloud 文档。
- [ ] 新设备下载车辆后必须由用户或当前仪表重新绑定；名称相同不能作为自动绑定依据。
- [ ] 合并规则为记录 ID 并集、较新修改覆盖、删除墓碑优先；无法安全裁决时保留冲突副本并提示用户，不静默覆盖。
- [ ] 支持 iCloud 不可用、离线编辑、延迟下载、容量错误、换机恢复和多车同时修改；手动同步保留为显式重试入口。

### 27.4 公共类型与迁移边界

- [ ] 新增定位用途、出发检查、轮胎/悬挂、保养任务、加油记录、路线风险、BLE 证据会话和云端车库文档的值类型；可跨并发边界的类型必须满足 `Sendable`。
- [ ] 轮胎/悬挂、保养、加油和云端记录均以车辆 UUID 归属；旧车库 schema 4 原样可读，迁移失败时保留原文件并返回可见错误。
- [ ] Watch context 和 PTT 位置包采用向后兼容的版本字段；未知字段忽略，缺失字段使用安全的不可用状态。
- [ ] 不新增第三方依赖、第二套蓝牙管理器、第二个天气服务入口、第二条 Watch 同步通道或新的发布 Target。

### 27.5 测试与发布验收

- [ ] 冷启动无停车、Widget、防盗、OBD、PTT 或 Live Activity 误激活；真实连接断开副作用即使重复回调也只执行一次。
- [ ] 定位用途组合、PTT 人数/位置/过期/限频、Live Activity 结束、行程车辆归属和云端墓碑具有自动测试。
- [ ] 1000 条行程/保养/加油记录和 100000 条 CAN JSONL 验证滚动、分析、取消、峰值内存和主线程卡顿。
...
## 37. Build 56 骑行数据专业分析实施记录（2026-09-08）

Build 56 继续使用营销版本 `2.0.8`。本轮将历史骑行从“摘要列表”升级为只读专业分析页：复用已有 `PTTripReport`、GPX 回放、原生遥测图表和 iCloud 缩略图，不引入新的 BLE/OBD 传输路径，也不修改三个稳定核心文件。

### 37.1 工作包与状态

| 状态 | ID | 实施结果 | 外部验收 |
| --- | --- | --- | --- |
| ✅ | `B56-01` | 历史列表改为紧凑摘要；点击卡片进入分析页，地图缩略图仍进入原有回放 | 需在不同屏幕尺寸、空历史和异步恢复时验证导航与布局 |
| ✅ | `B56-02` | 新增纯数据 `PTRideAnalysisBuilder`，计算时间、里程来源、速度、RPM、倾角、G 值、俯仰、海拔、胎压、打滑和事件事实 | 需用旧版/缺字段/异常浮点报告回归验证数值边界 |
| ✅ | `B56-03` | 详情页按类别切换速度、发动机、动态、环境和牵引图表；公共采样点最多 600 个并保留有限极值 | 需长途大轨迹、不同轨迹长度和低端设备滚动验证 |
| ✅ | `B56-04` | 仅用同一车辆最近最多 10 次有效历史，至少 3 次才显示平均基线与差值 | 需验证车辆切换、未绑定车辆、重复时间和历史删除场景 |
| ✅ | `B56-05` | 事件时间线可跳转回放位置；摘要卡与 JSON 分享采用脱敏快照 | 需检查分享渠道、取消分享、临时文件清理和隐私字段 |
| ✅ | `B56-06` | 数据质量区显示报告版本、各轨迹样本数、时间轴估算及缺失/长度不一致警告 | 需验证旧报告、无 GPX 和部分轨迹报告的可读性 |
| ✅ | `B56-07` | 新增分析指标、同车历史筛选、降采样极值和脱敏 JSON 的纯数据测试 | `build-for-testing` 已通过；XCTest 实际执行受 Pods 真机签名配置阻断，签名 Archive 和真实设备验证待补 |

### 37.2 设计与安全边界

- 分析页只读取 `PTTripReport`，不启动 BLE、OBD、CAN、Live Activity、导航或车辆控制。
- 不输出综合“骑行评分”，不把事实指标解释为驾驶能力或安全结论；缺失数据显示不可用并保留质量警告。
- 分享快照不包含坐标、VIN、车辆 ID、设备 ID、GPX 文件名或原始轨迹；完整原始 GPX 仍沿用用户主动导出的旧入口。
- 图表时间轴在没有逐点时间戳的旧报告中标为估算；各轨迹长度不一致时按公共索引对齐，不默认为采样同步。

### 37.3 已完成验证与剩余验收

- [x] 新增分析模型/构建器、详情控制器、事件回放跳转、列表入口、图表限流和多语言键。
- [x] 工程 Build 56 已写入，营销版本保持 `2.0.8`；三个稳定 BLE/OBD 核心文件零字节变化。
- [x] `Localizable.xcstrings` JSON 和 `git diff --check` 检查通过。
- [x] PTSpeed Debug generic build、Tests `build-for-testing` 已通过。
- [ ] XCTest 实际执行、签名发布、真机和长途大数据量验证待完成；当前真机测试被 Pods 依赖 target 未配置开发团队阻断。
