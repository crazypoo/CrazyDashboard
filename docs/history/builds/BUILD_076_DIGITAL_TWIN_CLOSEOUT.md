...
本次素材细化参考 XP400 官方公开车型的视觉特征：高位烟熏风挡、棱角化前脸、长座垫/低踏板、金色前叉和交叉辐条轮。实现仍采用项目内原创 SVG 与程序化 SceneKit 几何，不打包官方图片、Logo 或商标资源；因此它是接近官方比例与语言的近似模型，而不是官方授权模型。

2D 资源新增独立风挡层，并更新车身、灯组、轮圈和交叉辐条；3D 资源同步加入半透明风挡、前面板、踏板、前叉、排气和交叉辐条，使两种模式在轮廓和材质语言上保持一致。
...
| XP400 风格 2D 车身、风挡、灯组和交叉辐条细化 | ✅ |
| XP400 风格 3D 风挡、前脸、前叉、踏板和交叉辐条细化 | ✅ |
| 2D/3D 当前配置固定为 XP400 | ✅ |
